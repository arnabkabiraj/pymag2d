...
relx_settings = {'IOPT':3,'EDIFF':1e-6,'NELM':200,'EDIFFG':-0.02,'NSW':400,'KPAR':4}
relx_settings_light = {'EDIFF':1e-5,'NELM':400}
stat_settings = {'LVHAR':True,'LVTOT':False,'KPAR':4,'NSIM':4,'NELM':1500}
stat_settings_light = stat_settings.copy()
stat_settings_light.update({'ISYM':2})
...
for file in everything:
  ...
  if '.cif' in file and os.path.exists(root_path+'/'+file):
    ...
    x = relx_gen('mpirun -np '+str(sys.argv[3])+' vasp_std_631_acc',file,user_incar_settings_relx=relx_settings,dipole_correction=True,xc='R2SCAN',
      randomise_cmd=False,potcar_provided={'W':'W_sv'},mag_species=magnetic_list,max_errors=10)
    ...
    x = stat_gen('mpirun -np '+str(sys.argv[4])+' vasp_std_631_acc','./initial_relx/CONTCAR',user_incar_settings_stat=stat_settings_light,dipole_correction=False,
      xc='R2SCAN',randomise_cmd=False,potcar_provided={'W':'W_sv'},mag_species=magnetic_list,max_errors=10)
    ...
    x = run_heisenberg('mpirun -np '+str(sys.argv[5])+' vasp_std_631_acc','./initial_stat/POSCAR',[2,4,1],max_neigh=4,relx=False,dipole_correction=False,
      ...
      max_errors_relx=10,max_errors_stat=10)
    ...
    [S,J1,J2,J3,J4] = fit_heisenberg(with_anisotropy=False,max_neigh=4,mag_species=magnetic_list)
    ...
    d1plane = run_calc_dmi_hex_NN_inplane('mpirun -np '+str(sys.argv[6])+' vasp_ncl_631_acc',user_incar_settings=dmi_settings,
      dipole_correction=False,icharg_scan=2,xc='R2SCAN',randomise_cmd=False,user_potcar_settings={'W':'W_sv'},
      constrain=True,lamb=50,mag_species=magnetic_list,max_errors=3,rm_wavecar=True)

    if d1plane==None:
      msg = 'seems the DMI calculations failed, retrying with lambda=10 instead of the regular 50'
      log(msg)
      if os.path.exists(path+'/DMI_hex_NN_inplane') and not os.path.exists(path+'/DMI_hex_NN_inplane_flipped_1'):
        os.rename(path+'/DMI_hex_NN_inplane',path+'/DMI_hex_NN_inplane_flipped_1')
      dmi_settings.update({'NELM':750})
      d1plane = run_calc_dmi_hex_NN_inplane('mpirun -np '+str(sys.argv[6])+' vasp_ncl_631_acc',user_incar_settings=dmi_settings,
        dipole_correction=False,icharg_scan=2,xc='R2SCAN',randomise_cmd=False,user_potcar_settings={'W':'W_sv'},
        constrain=True,lamb=10,mag_species=magnetic_list,max_errors=3,rm_wavecar=True)
    ...
    msg = 'the d1plane/J1 ratio from collinear calculations is '+str(ratio)
    log(msg)
    ...
    x = run_anisotropy('mpirun -np '+str(sys.argv[7])+' vasp_ncl_631_acc',kpar=3,nsim=4,icharg_scan=1,xc='R2SCAN',
      randomise_cmd=False,potcar_provided={'W':'W_sv'},mag_species=magnetic_list,max_errors=5,
      nelm=1500,lcharg=False,lwave=False,rm_chgcar=True,rm_wavecar=True)
    ...
    [S,J1,J2,J3,J4,K1z,K2z,K3z,K4z,Az] = fit_heisenberg_anisotropic(max_neigh=4,mag_species=magnetic_list)
    ...
    msg = 'the final d1plane/J1 ratio from noncollinear calculations is '+str(ratio)
    log(msg)

    os.chdir(root_path)
    msg = 'all calculations finished for material '+file[:-4]+' which is a potentially good material for skyrmion formation'
    log(msg)
